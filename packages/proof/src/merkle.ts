...
export async function createStoredMerkleInclusionProof(input: {
  ...
  const auditPlans = buildStoredAuditPathPlans(
    0,
    input.treeSize,
    input.leafIndex,
  );
...
export async function verifyMerkleConsistencyProof(input: {
  ...
  try {
    ...
    const path = isPowerOfTwo(input.firstTreeSize)
      ? [firstRoot, ...proof]
      : proof;
...
async function loadPlannedNodes(
  plans: MerkleRangePlan[],
  loader: MerkleNodeLoader,
) {
