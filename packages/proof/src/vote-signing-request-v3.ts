...
export type VerifiedVoteSigningRequestBatchItemV3 =
  VoteSigningRequestBatchItemV3 & {
...
export function buildVoteSigningIntentV3(input: {
  ...
  const preparedAt =
    input.preparedAt instanceof Date
      ? input.preparedAt
      : new Date(input.preparedAt);
...
export function assertVoteSigningIntentV3(
  ...
  if (
    ...
    (value.previousEventSha256 !== null &&
      !digest(value.previousEventSha256)) ||
...
export function voteSigningRequestBatchSha256V3(
  batch: VoteSigningRequestBatchV3,
) {
