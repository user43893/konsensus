...
async function fixture() {
  ...
  const questionKeySpki = base64UrlEncode(
    new Uint8Array(
      await crypto.subtle.exportKey("spki", questionKey.publicKey),
    ),
  );
...
describe("private V3 tally", () => {
  ...
  test("contains no reusable registration identity or source-to-vote link", async () => {
    ...
    requiredFirst(linked.sourcePositions, "linked source").personId =
      "known-lawyer";
    expect(() => buildTallyInputSetV3(linked)).toThrow(
      /personId is not allowed/,
    );

    const extendedAuthorization = structuredClone(inputSet) as typeof inputSet;
    Object.assign(
      requiredFirst(
        extendedAuthorization.questionVotingAuthorizations,
        "authorization",
      ).payload,
  ...
  test("rejects an authorization from a different pinned question epoch", async () => {
    ...
    await expect(
      replayTallyV3(value.tallyPolicy, value.inputSet),
    ).rejects.toThrow(/different nullifier epochs/);
  });
  ...
  test("fails closed if an authorization or question-log leaf is omitted", async () => {
    ...
    await expect(
      replayTallyV3(value.tallyPolicy, noAuthorization),
    ).rejects.toThrow(/no public question authorization/);
    ...
    await expect(
      replayTallyV3(value.tallyPolicy, incompleteLog),
    ).rejects.toThrow(/prefix length|not contiguous|is not logged/);
  });
  ...
  test("verifies attestation policy, question-key signatures, receipts and snapshot signatures", async () => {
    ...
    requiredFirst(
      value.inputSet.ballotManifests,
      "ballot manifest",
    ).signature.value = value.inputSet.questionManifest.signature.value;
    ...
    requiredFirst(badEvent.voteEvents, "bad event").envelope.signature.value =
      "AA";
