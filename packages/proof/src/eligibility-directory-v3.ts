...
import { type EligibilityDecisionV3, type SignedPayload } from "./types";
import {
  assertEligibilityDecisionV3,
  verifySignedPayloadHash,
} from "./validate";
...
export async function verifyActiveEligibilityDirectoryRecordProofIntegrityV3(
  ...
  if (
    !(await verifyActiveEligibilityDirectoryRecordIntegrityV3(proof.record))
  ) {
...
export function assertActiveEligibilityDirectoryLeafV3(
  ...
  literal(
    object.schema,
    ACTIVE_ELIGIBILITY_DIRECTORY_LEAF_SCHEMA_V3,
    "$.schema",
  );
  stableId(object.publicVoterId, "$.publicVoterId");
...
export function assertActiveEligibilityDirectoryCheckpointV3(
  ...
  literal(
    object.schema,
    ACTIVE_ELIGIBILITY_DIRECTORY_CHECKPOINT_SCHEMA_V3,
    "$.schema",
  );
...
export function assertActiveEligibilityDirectoryBundleV3(
  ...
  literal(
    object.schema,
    ACTIVE_ELIGIBILITY_DIRECTORY_BUNDLE_SCHEMA_V3,
    "$.schema",
  );
  ...
  for (const [index, record] of object.records.entries()) {
    try {
      ...
      fail(
        `$.records[${index}] is invalid: ${validationMessages(error).join("; ")}`,
      );
    }
  }
}
...
function assertSortedUniqueLeaves(
  leaves: readonly ActiveEligibilityDirectoryLeafV3[],
) {
  for (let index = 0; index < leaves.length; index += 1) {
    ...
    if (
      !current ||
      (previous &&
        previous.publicVoterId.localeCompare(current.publicVoterId) >= 0)
    ) {
      throw new TypeError(
