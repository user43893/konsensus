...
async function fixture() {
  ...
  const identityAttestationPolicy = buildIdentityAttestationPolicyV3({
    ...
    expectedEnvironment: {
      EXAMPLE_WEBAUTHN_ORIGIN: "https://verify.example.test",
      EXAMPLE_WEBAUTHN_RP_ID: "verify.example.test",
    },
    allowDebug: false,
...
describe("EligibilityAssertionV3", () => {
  test("binds exact identity-attestation policy material", async () => {
    ...
    expect(assertion.identityAttestation.policy.imageDigest).toBe(
      "sha256:image-a",
    );
  });
  ...
  test("detects a policy payload changed without changing its signed hash", async () => {
    const assertion = await fixture();
    assertion.identityAttestation.policy.expectedEnvironment.EXAMPLE_WEBAUTHN_RP_ID =
