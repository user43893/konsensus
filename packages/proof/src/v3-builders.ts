...
export function buildProtocolBindingV3(input: {
  ...
  const binding: ProtocolBindingV3 = {
    ...
    eligibilityPolicy: policyReference(
...
export function buildRegistryEvidenceV3(input: {
  ...
  return {
    ...
    reviewerAuthority: requiredString(
...
export function buildActiveEligibilityDirectoryV3(input: {
  ...
  const entries = input.entries
    .map((entry) => ({
      publicVoterId: requiredString(
        entry.publicVoterId,
        "entries.publicVoterId",
      ),
...
export async function buildQuestionVotingAuthorizationPayloadV3(input: {
  ...
  const publicKeySpkiSha256 = await sha256Base64Url(
...
export async function buildQuestionVotingAuthorizationV3(input: {
  ...
  const authorization: QuestionVotingAuthorizationV3 = {
    ...
    attestationToken: requiredString(
      input.attestationToken,
      "attestationToken",
    ),
  };
  const integrity =
...
function qualificationSubject(
  value: QualificationSubjectV3,
): QualificationSubjectV3 {
  return {
