...
export const TALLY_POLICY_SCHEMA_V3 =
  "qualified-voting.tally-policy.v3" as const;
export const TALLY_INPUT_SET_SCHEMA_V3 =
  "qualified-voting.tally-input-set.v3" as const;
export const TALLY_SNAPSHOT_SCHEMA_V3 =
  "qualified-voting.tally-snapshot.v3" as const;
...
export async function replayTallyV3(
  ...
  for (const entry of latestByNullifier.values()) {
    const event = entry.envelope.payload;
    const receipts = graph.receiptsByEventHash.get(
      entry.envelope.payloadSha256,
    );
    const latestReceipt = receipts?.at(-1);
    if (
      !latestReceipt ||
      latestReceipt.envelope.payload.status !==
        policy.directVote.countedDecision ||
  ...
  const resultChoices: TallyChoiceResultV3[] = choices.map((choice) => {
    ...
    const weightedCount = matching.reduce(
  ...
  const result: TallyResultV3 = {
    ...
    totalWeight,
    directVoteCount: counted.filter((entry) => entry.kind === "direct_vote")
      .length,
    sourcePositionCount: counted.filter(
      (entry) => entry.kind === "source_position",
    ).length,
    disputedCount: inputSet.sourcePositions.filter(
      (position) =>
        position.questionId === inputSet.questionId &&
...
export async function verifyTallySnapshotStructureV3(input: {
  ...
  try {
    ...
    if (
      ...
      snapshot.questionManifestSha256 !==
        input.inputSet.questionManifest.payloadSha256
    ) {
    ...
    if (
      ...
      canonicalizeJson(snapshot.treeHead) !==
        canonicalizeJson(input.inputSet.treeHead)
    ) {
    ...
    if (
      ...
      snapshot.inputSet.sourcePositionCount !==
        replay.result.sourcePositionCount
    ) {
...
export function assertTallyPolicyV3(
  value: unknown,
): asserts value is TallyPolicyV3 {
  const errors: string[] = [];
  if (!record(value)) {
  ...
  if (!record(value.directVote)) {
    ...
    if (
      ...
      errors.push(
        "policy.directVote.countedEventTypes must be exactly cast,replace",
      );
    }
  }
  if (!Array.isArray(value.sourcePositionRules)) {
    ...
    for (const [index, candidate] of value.sourcePositionRules.entries()) {
      ...
      if (candidate.credentialClasses !== null) {
        stringArray(
          candidate.credentialClasses,
          `${path}.credentialClasses`,
          errors,
        );
      }
...
export function assertTallySnapshotV3(
  ...
  if (!record(value.policy)) {
    ...
    nonEmpty(value.policy.policyId, "snapshot.policy.policyId", errors);
    digest(value.policy.payloadSha256, "snapshot.policy.payloadSha256", errors);
    captureAssertion(
      () => assertTallyPolicyV3(value.policy?.payload),
  ...
  if (!record(value.inputSet)) {
    ...
    digest(
      value.inputSet.payloadSha256,
      "snapshot.inputSet.payloadSha256",
      errors,
    );
    integer(
...
async function validateReplayGraphV3(
  policy: TallyPolicyV3,
  input: TallyInputSetV3,
) {
  const errors: string[] = [];
  ...
  if (
    ...
    errors.push(
      "selected ballot manifest does not identify the target question",
    );
  }
  checkBindingPolicy(
  ...
  const selectedMeaning = canonicalizeJson(
    selectedManifest.payload.meaning.choices,
  );
  ...
  for (const authorization of input.questionVotingAuthorizations) {
    const integrity =
      await verifyQuestionVotingAuthorizationV3Integrity(authorization);
    if (!integrity.ok) {
      errors.push(
        ...integrity.errors.map(
          (error) =>
            `question authorization ${authorization.payloadSha256}: ${error}`,
        ),
      );
      continue;
    }
    if (authorizationsByHash.has(authorization.payloadSha256)) {
      errors.push(
        `duplicate question authorization ${authorization.payloadSha256}`,
      );
    }
    ...
    if (
      authorization.payload.eligibility.claim !==
      policy.qualifiedVote.eligibilityClaim
    ) {
      errors.push(
  ...
  for (const entry of input.voteEvents) {
    ...
    if (!authorization) {
      errors.push(
        `vote event ${event.eventId} has no public question authorization`,
      );
    } else {
      ...
      if (!binding.ok) {
        errors.push(
          ...binding.errors.map(
            (error) => `vote event ${event.eventId}: ${error}`,
          ),
        );
      }
    }
    ...
    if (
      ...
      errors.push(
        `vote event ${event.eventId} has no matching ballot manifest`,
      );
    } else if (
      event.choiceId !== null &&
      !manifest.payload.meaning.choices.some(
    ...
    if (
      authorization &&
      manifest &&
      authorization.payload.nullifierKeyEpoch !==
        manifest.payload.nullifierKeyEpoch
    ) {
      errors.push(
  ...
  for (const [nullifier, chain] of chains) {
    chain.sort(
      (left, right) =>
        left.envelope.payload.sequence - right.envelope.payload.sequence,
    );
  ...
  for (const receipt of input.voteReceipts) {
    ...
    if (!eventsByHash.has(payload.voteEventSha256)) {
      errors.push(
        `vote receipt ${payload.receiptId} references an unpublished event`,
      );
    }
  ...
  for (const [eventHash, receipts] of receiptsByEventHash) {
    receipts.sort(
      (left, right) => receiptSequence(left) - receiptSequence(right),
    );
    for (let index = 0; index < receipts.length; index += 1) {
      ...
      if (
        ...
        (index === 0 &&
          payload.schema !== "qualified-opinion.vote-acceptance.v3") ||
        (index > 0 &&
          (payload.schema !== "qualified-opinion.vote-adjudication.v3" ||
  ...
  for (let index = 0; index < input.questionLogLeaves.length; index += 1) {
    ...
    if (leaf.leafIndex !== index) {
      errors.push(
        `question log prefix is not contiguous at leaf index ${index}`,
      );
    }
  ...
  if (recomputedRoot !== input.checkpoint.rootHash) {
    errors.push(
      "question log prefix does not recompute to the checkpoint root",
    );
  }
  for (const entry of input.voteEvents) {
  ...
  for (const entry of input.voteReceipts) {
    ...
    if (!leafByEntry.has(key)) {
      errors.push(
        `vote receipt ${entry.envelope.payload.receiptId} is not logged`,
      );
    }
  }
  ...
  for (const position of input.sourcePositions) {
    if (position.questionId !== input.questionId) {
      errors.push(
        `source position ${position.positionId} is for another question`,
      );
    }
    const matches = matchingSourceRules(policy, position);
    if (matches.length > 1) {
      errors.push(
        `source position ${position.positionId} matches multiple rules`,
      );
    }
  }
...
async function checkEnvelopeHash<T>(
  ...
  if (
    (await canonicalJsonSha256(envelope.payload)) !== envelope.payloadSha256
  ) {
...
function assertVoteReceipt(value: unknown): asserts value is VoteReceiptV3 {
  if (
    record(value) &&
    value.schema === "qualified-opinion.vote-acceptance.v3"
  ) {
...
function isSignedEnvelope<T>(
  value: T | SignedPayload<T>,
): value is SignedPayload<T> {
...
function questionLogLeaf(value: unknown, path: string, errors: string[]) {
  ...
  if (
    value.entryType !== "vote_event" &&
    value.entryType !== "vote_adjudication"
  ) {
...
function sourcePosition(value: unknown, path: string, errors: string[]) {
  ...
  if (
    value.credentialClass !== null &&
    typeof value.credentialClass !== "string"
  ) {
...
function receiptSequence(receipt: TallyVoteReceiptInputV3) {
  return receipt.envelope.payload.schema ===
    "qualified-opinion.vote-acceptance.v3"
    ? 1
...
function canonicalStrings(values: string[]) {
  return [
    ...new Set(values.map((value) => requiredString(value, "string"))),
...
function captureAssertion(action: () => void, path: string, errors: string[]) {
  try {
    ...
    errors.push(
      ...errorMessages(error).map((message) => `${path}: ${message}`),
    );
  }
}
