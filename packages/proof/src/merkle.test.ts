...
describe("RFC 6962 Merkle tree", () => {
  ...
  test("creates and verifies consistency proofs for every tree-size pair", async () => {
    const exhaustiveLeaves = Array.from(
      { length: 32 },
      (_, index) => `leaf-${index}`,
    );
  ...
  test("rejects every mutated proof node and root", async () => {
    ...
    for (
      ...
      for (let index = 0; index < proof.auditPath.length; index += 1) {
        ...
        expect(
          await verifyMerkleConsistencyProof({ ...proof, auditPath }),
        ).toBe(false);
      }
    }
  }, 30_000);
  ...
  test("rejects invalid consistency sizes, roots and boundary proofs", async () => {
    ...
    for (const sizes of invalidSizes) {
      expect(await verifyMerkleConsistencyProof({ ...proof, ...sizes })).toBe(
        false,
      );
    }
  ...
  test("rejects invalid consistency proof creation sizes", async () => {
    await expect(createMerkleConsistencyProof(leaves, -1)).rejects.toThrow(
      RangeError,
    );
    await expect(createMerkleConsistencyProof(leaves, 1.5)).rejects.toThrow(
      RangeError,
    );
    await expect(createMerkleConsistencyProof(leaves, 6)).rejects.toThrow(
      RangeError,
    );
  });
});
