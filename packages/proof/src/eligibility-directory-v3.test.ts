...
async function recordFixture(input: {
  ...
  const identityPolicy = buildIdentityAttestationPolicyV3({
    ...
    expectedEnvironment: {
      EXAMPLE_WEBAUTHN_ORIGIN: "https://example.test",
      EXAMPLE_WEBAUTHN_RP_ID: "example.test",
    },
    allowDebug: false,
...
describe("active eligibility directory V3", () => {
  test("commits to one deterministic complete sorted current set", async () => {
    ...
    expect(
      await verifyActiveEligibilityDirectoryBundleIntegrityV3(bundle),
    ).toEqual({ ok: true, value: bundle });
  });
  ...
  test("proves one record without disclosing every record body", async () => {
    ...
    expect(
      await verifyActiveEligibilityDirectoryRecordProofIntegrityV3(proof),
    ).toEqual({ ok: true, value: proof });
    const tampered = structuredClone(proof);
    tampered.auditPath[0] = digest(99);
    expect(
      (await verifyActiveEligibilityDirectoryRecordProofIntegrityV3(tampered))
        .ok,
    ).toBe(false);
  });
  ...
  test("record digests cover the exact canonical record", async () => {
    ...
    expect(await activeEligibilityDirectoryRecordSha256V3(record)).toHaveLength(
      43,
    );
  });
});
