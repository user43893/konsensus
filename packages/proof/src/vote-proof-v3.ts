...
export async function verifyPublicVoteProofBundleV3Integrity(
  ...
  const errors: string[] = [];
  const authorizationIntegrity =
    await verifyQuestionVotingAuthorizationV3Integrity(
  ...
  const manifestHash = await canonicalJsonSha256(
  ...
  if (
    ...
    !bundle.questionManifest.payload.meaning.choices.some(
      (choice) =>
        choice.id === bundle.voteEvent.payload.choiceId && choice.isCounted,
    )
  ) {
...
function assertSignedEnvelope(
  ...
  const envelope = exactObject(value, path, [
    "payload",
    "payloadSha256",
    "signature",
  ]);
