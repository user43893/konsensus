...
export async function buildEligibilityAssertionV3(input: {
  ...
  const payload: EligibilityAssertionV3 = {
    ...
    identityProof: {
      proofId: requiredString(
        input.identityProof.proofId,
        "identityProof.proofId",
      ),
    ...
    rootKey: {
      credentialId: requiredString(
        input.rootKey.credentialId,
        "rootKey.credentialId",
      ),
...
export function validateEligibilityAssertionV3(
  ...
  if (!isRecord(value.identityProof)) {
    ...
    digest(
      value.identityProof.payloadSha256,
      "$.identityProof.payloadSha256",
      errors,
    );
  }
  if (!isRecord(value.rootKey)) {
    ...
    digest(
      value.rootKey.publicKeySpkiSha256,
      "$.rootKey.publicKeySpkiSha256",
      errors,
    );
  }
  ...
  if (!isRecord(value.qualification)) {
    ...
    if (
      ...
      errors.push(
        "$.qualification.policy must equal binding.eligibilityPolicy",
      );
    }
  }
  if (!isRecord(value.registryEvidence)) {
    ...
    if (!isRecord(evidence)) {
      ...
      nonEmpty(
        evidence.reviewId,
        "$.registryEvidence.payload.reviewId",
        errors,
      );
      nonEmpty(
      ...
      if (
        ...
        evidenceSubject &&
        canonicalizeJson(qualificationSubject) !==
          canonicalizeJson(evidenceSubject)
      ) {
        errors.push(
      ...
      httpUrl(
        evidence.recordUrl,
        "$.registryEvidence.payload.recordUrl",
        errors,
      );
      nonEmpty(
      ...
      timestamp(
        evidence.checkedAt,
        "$.registryEvidence.payload.checkedAt",
        errors,
      );
    }
  }
...
function collectIdentityAttestationPolicyErrors(value: unknown, path: string) {
  ...
  literal(
    value.issuer,
    GOOGLE_CONFIDENTIAL_SPACE_ISSUER_V3,
    `${path}.issuer`,
    errors,
  );
  for (const key of [
  ...
  if (
    ...
    for (const [name, environmentValue] of Object.entries(
