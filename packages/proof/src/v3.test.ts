...
function binding(): ProtocolBindingV3 {
  return buildProtocolBindingV3({
    instanceId: "org.example.qualified-opinions",
...
describe("question voting authorization V3", () => {
  ...
  test("derives a fresh question key ID from the decoded SPKI", async () => {
    ...
    expect(first.payload.questionKey.keyId).not.toBe(
      second.payload.questionKey.keyId,
    );
  });

  test("rejects unknown member-linking fields at every exact boundary", async () => {
    const authorization = await authorizationFixture();
    const topLevel = structuredClone(
      authorization,
    ) as QuestionVotingAuthorizationV3 & { publicVoterId?: string };
  ...
  test("detects payload tampering and a falsely labelled ES256 key", async () => {
    ...
    falseKey.payload.questionKey.publicKeySpki =
      base64UrlEncode(falseSpkiBytes);
    ...
    const falseKeyResult =
      await verifyQuestionVotingAuthorizationV3Integrity(falseKey);
