...
export async function expectedQuestionVotingAuthorizationAttestationNonceV3(
  ...
  const integrity =
    await verifyQuestionVotingAuthorizationPayloadV3Integrity(payload);
  if (!integrity.ok) {
    throw new TypeError(
...
export async function verifyQuestionVotingAuthorizationV3Integrity(
  ...
  const payloadIntegrity =
    await verifyQuestionVotingAuthorizationPayloadV3Integrity(
      structural.value.payload,
    );
  const errors = payloadIntegrity.ok
    ? []
    : rebase(payloadIntegrity.errors, "$.payload");
  const expectedPayloadSha256 = await canonicalJsonSha256(
    structural.value.payload,
  );
...
async function questionKeyIntegrityErrors(
  ...
  if (spkiBytes) {
    ...
    if (!(await isP256Spki(spkiBytes))) {
      errors.push(
        `${path}.questionKey.publicKeySpki must encode a P-256 public key`,
      );
    }
  }
  ...
  if (
    (await sha256Base64Url(witness.canonicalEnvelope)) !==
...
export function validateVoteEventV3(
  value: unknown,
): ValidationResult<VoteEventV3> {
  return result(value, collectVoteEventErrors(value, "$"));
}

export function assertVoteEventV3(
  value: unknown,
): asserts value is VoteEventV3 {
...
export async function verifyVoteEventV3AuthorizationBinding(input: {
  ...
  if (event.value.questionNullifier !== payload.questionNullifier) {
    errors.push(
      "$.event.questionNullifier must equal authorization questionNullifier",
    );
  }
  if (event.value.authorizationSha256 !== authorization.value.payloadSha256) {
    errors.push(
      "$.event.authorizationSha256 must equal authorization payloadSha256",
    );
  }
  if (event.value.questionKeyId !== payload.questionKey.keyId) {
    errors.push(
      "$.event.questionKeyId must equal authorization questionKey.keyId",
    );
  }
  if (
    canonicalizeJson(event.value.binding) !== canonicalizeJson(payload.binding)
  ) {
  ...
  if (eventTime < authorizationStart || eventTime >= authorizationEnd) {
    errors.push(
      "$.event.issuedAt must fall within the authorization validity window",
    );
  }
...
function collectAuthorizationErrors(value: unknown, path: string) {
  ...
  errors.push(
    ...collectAuthorizationPayloadErrors(object.payload, `${path}.payload`),
  );
...
function collectAuthorizationPayloadErrors(value: unknown, path: string) {
  ...
  positiveInteger(
    object.nullifierKeyEpoch,
    `${path}.nullifierKeyEpoch`,
    errors,
  );
  ...
  if (questionKey) {
    literal(
      questionKey.algorithm,
      "ES256",
      `${path}.questionKey.algorithm`,
      errors,
    );
    digest(questionKey.keyId, `${path}.questionKey.keyId`, errors);
    base64Value(
      questionKey.publicKeySpki,
      `${path}.questionKey.publicKeySpki`,
      errors,
    );
    digest(
  ...
  if (eligibility) {
    ...
    if (
      ...
      errors.push(
        `${path}.eligibility.policy must equal binding.eligibilityPolicy`,
      );
    }
  }
...
function collectVoteEventErrors(value: unknown, path: string) {
  ...
  if (object.sequence === 1) {
    ...
    if (object.eventType === "cast") {
      errors.push(
        `${path}.eventType must be replace or withdraw after sequence 1`,
      );
    }
  }
  return errors;
...
function collectBallotErrors(value: unknown, path: string) {
      ...
      for (const [index, choice] of meaning.choices.entries()) {
        ...
        if (typeof item.displayOrder === "number") {
          if (item.displayOrder < previousOrder) {
            errors.push(
              `${path}.meaning.choices must be ordered by displayOrder`,
            );
          }
  ...
  if (presentation) {
    ...
    nonEmpty(
      ...
      `${path}.presentation.questionText`,
      errors,
    );
    nullableString(
...
function collectPolicyReference(
  value: unknown,
  path: string,
  errors: string[],
) {
