...
export async function verifyP256SignedPayload<T>(input: {
  ...
  if (
    ...
    (input.expectedKeyId &&
      input.envelope.signature.keyId !== input.expectedKeyId) ||
...
export async function verifyTallySnapshotCryptographicallyV3(input: {
      ...
      ? [
          {
            ...
            envelope:
              input.expectedLatestQuestionTreeHead as SignedPayload<unknown>,
            signedAt: input.expectedLatestQuestionTreeHead.payload.issuedAt,
            expectedKeyId:
              input.expectedLatestQuestionTreeHead.payload.issuerKeyId,
          },
        ]
      : []),
    ...
    try {
      const firstPublishedAt =
        input.inputSet.voteEvents
          ...
          .map(
            (event) =>
              firstPublishedByPayloadHash.get(event.payloadSha256) ??
              checkpointTime,
          )
  ...
  for (const entry of input.inputSet.voteEvents) {
    ...
    if (
      ...
      entry.envelope.signature.keyId !==
        authorization.payload.questionKey.keyId ||
...
function trustWindowContainsTimestamp(
  ...
  const validUntil =
    key.validUntil === null ? null : Date.parse(key.validUntil);
  return (
